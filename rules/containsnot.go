...
// ContainsNot checks if the input string does not contain the given substring.
//
// The function expects exactly one argument (substring) provided in the `args` map.
// It evaluates the argument and checks whether the input string does not contain the evaluated substring.
// If the input or substring is not a valid string, or if the input string contains the substring,
...
func ContainsNot(input any, obj any, arguments map[string]args.Arg) error {
	if len(arguments) != 1 {
		return fmt.Errorf("containsnot expects 1 argument, got %d", len(arguments))
	}
	...
	if err != nil {
		return fmt.Errorf("unsupported type for containsnot argument: %w", err)
	}

	if len(value) == 0 || len(contained) == 0 {
		return fmt.Errorf("containsnot validation failed: empty string")
	}

	// Compare values
	if strings.Contains(value, contained) {
		return fmt.Errorf("containsnot validation failed: %s contains %s", value, contained)
	}
